// Importa hooks e icones necessarios
...
/* Componente Header: renderiza o cabecalho da aplicacao, com menu, busca, perfil e logout */
const Header = ({ busca, setBusca, filtro, setFiltro, buscarFilmes, filmesFavoritos }) => {
  const { user, logout } = useAuth();
  const navigate = useNavigate();

  // Estado que armazena os dados do usuario logado (nome e avatar)
  const [userData, setUserData] = useState(null);

  /* useEffect que busca os dados do usuario no banco (supabase) ao montar ou mudar o user */
  useEffect(() => {
  ...
  // Estado para abrir/fechar menu lateral mobile e card de usuario
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [showUserCard, setShowUserCard] = useState(false);

  /* Funçao para logout e redirecionamento apos sair */
  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-[#1a1a1a] md:bg-gradient-to-r md:from-[#1a1a1a] md:to-[#300818] shadow-md">
      <div className="px-4 md:px-8 pt-4 pb-4">
        {/* Topo com logo e nome da aplicacao */}
        <div className="flex items-center justify-between">
          ...
          {/* Perfil visivel no modo desktop */}
          <div className="hidden md:flex items-center space-x-4 relative">
            ...
            >
              <div className="relative flex items-center justify-center w-8 h-8">
                ...
                {/* brilho animado no fundo do icone */}
            ...
            {/* Card de perfil (visivel se showUserCard = true) */}
            {showUserCard && (
              <>
                {/* Clique fora fecha o card */}
                <div
        ...
        {/* Menu mobile expandido com busca */}
        {isMenuOpen && (
        ...
        {/* Barra de busca completa no desktop */}
